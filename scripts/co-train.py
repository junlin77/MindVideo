...
def train_unet_loop(config, unet, vae, fmri_encoder, noise_scheduler, optimizer_unet, optimizer_fmri, train_dataloader, lr_scheduler, device, dtype):
    ...
    # Prepare everything
    unet, vae, fmri_encoder, optimizer_unet, optimizer_fmri, train_dataloader, lr_scheduler = accelerator.prepare(
        unet, vae, fmri_encoder, optimizer_unet, optimizer_fmri, train_dataloader, lr_scheduler
    )

    vae.eval()
    global_step = 0

    # Now you train the unet and fMRI encoder
    for epoch in range(config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                accelerator.clip_grad_norm_(fmri_encoder.parameters(), 1.0)
                
                # Step the optimizers for both models
                optimizer_unet.step()
                optimizer_fmri.step()
                lr_scheduler.step()
