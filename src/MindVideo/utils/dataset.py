...
def create_BOLD5000_dataset(path='../data/BOLD5000', patch_size=16, fmri_transform=identity,
            image_transform=identity, subjects = ['CSI1', 'CSI2', 'CSI3', 'CSI4'], include_nonavg_test=False, include_image_caption=False):
    ...
    for sub in subjects:
        ...
        # load caption
        caption_data_sub = [caption_dict[name] for name in img_files]
        ...
        if include_nonavg_test:
            ...
            test_caption += [caption_data_sub[idx] for idx in test_idx_flatten]
        ...
        caption_train_major.append(train_caption)
        caption_test_major.append(test_caption)
    ...
    caption_train_major = np.concatenate(caption_train_major, axis=0)
    caption_test_major = np.concatenate(caption_test_major, axis=0)

    num_voxels = fmri_train_major.shape[-1]
    
    if include_image_caption==False:
        if isinstance(image_transform, list):
        ...
        if isinstance(image_transform, list):
            return (FmriImageCaptionDataset(fmri_train_major, img_train_major, caption_train_major, fmri_transform, image_transform[0], num_voxels), 
                    FmriImageCaptionDataset(fmri_test_major, img_test_major, caption_test_major, torch.FloatTensor, image_transform[1], num_voxels))
        else:
            return (FmriImageCaptionDataset(fmri_train_major, img_train_major, caption_train_major, fmri_transform, image_transform, num_voxels), 
                    FmriImageCaptionDataset(fmri_test_major, img_test_major, caption_test_major, torch.FloatTensor, image_transform, num_voxels))
...
class FmriImageCaptionDataset(Dataset):
    def __init__(self, fmri, image, caption, fmri_transform=identity, image_transform=identity, caption_transform=identity, num_voxels=0):
        self.fmri = fmri
        self.image = image
        ...
        self.image_embeddings = self.image_encoder(torch.from_numpy(image)).last_hidden_state
        self.text_encoder = CLIPTextModel.from_pretrained("openai/clip-vit-base-patch32")
        self.caption_embeddings =  self.text_encoder(torch.from_numpy(caption)).last_hidden_state
...
def create_Wen_dataset(path='../data/Wen', patch_size=16, fmri_transform=identity,image_transform=identity, 
    ...
    for sub in subjects:
        ...
        roi_len.append(train_data_.shape[-1])

        train_data.append(train_data_)
        test_data.append(test_data_)

    train_fmri = np.concatenate([pad_to_length(i, max(roi_len)) for i in train_data], axis=0)
    test_fmri = np.concatenate([pad_to_length(i, max(roi_len)) for i in test_data], axis=0)
  
    num_voxels = train_fmri.shape[-1]

    # sample the test_fmri for faster evalation
    # test_fmri = test_fmri[:,::test_step,:]
    # video_frames_test = video_frames_test[:,::test_step,:,:,:,:]
    # text_test = text_test[:,::test_step,:]
   
    if isinstance(image_transform, list):
        return (Wen_dataset(train_fmri, video_frames_train, text_train, None,
                            fmri_transform, image_transform[0], num_voxels, window_size), 
                Wen_dataset(test_fmri, video_frames_test, text_test, test_seg_id,
                            torch.FloatTensor, image_transform[1], num_voxels, window_size))
    else:
        return (Wen_dataset(train_fmri, video_frames_train, text_train, None,
                            fmri_transform, image_transform, num_voxels, window_size), 
                Wen_dataset(test_fmri, video_frames_test, text_test, test_seg_id,
                            torch.FloatTensor, image_transform, num_voxels, window_size))

def create_Wen_test_data_only(path='../data/Wen', patch_size=16, fmri_transform=identity,image_transform=identity, 
            subjects = ['subject1', 'subject2', 'subject3'], window_size=0, fps=3, **kwargs):
    ...
    for sub in subjects:
        # (5, 240, 3731)
        test_data_ = pad_to_patch_size(normalize(np.load(os.path.join(path,'preprocessed', 
                                                f'fmri_test_{sub}.npy')), mean=0.0, std=1.0), patch_size)
        roi_len.append(test_data_.shape[-1])
    ...
    if isinstance(image_transform, list):
        return Wen_dataset(test_fmri, video_frames_test, text_test, test_seg_id,
                            torch.FloatTensor, image_transform[1], num_voxels, window_size)
    else:
        return Wen_dataset(test_fmri, video_frames_test, text_test, test_seg_id,
                            torch.FloatTensor, image_transform, num_voxels, window_size)

class Wen_dataset(Dataset):
    def __init__(self, fmri, video, text, test_seg_id = None,
                 fmri_transform=identity, 
                 image_transform=identity, 
                 num_voxels=0,
                 window_size=0, **kwargs):
        super(Wen_dataset, self).__init__()
        
        self.t = video.shape[1] # sec per video (t)
        self.fmri = rearrange(fmri, 's t v -> (s t) v')
        self.video = rearrange(video, 's t v h w c -> (s t) v h w c')
        self.text = rearrange(text, 's t v -> (s t) v')
        self.uncon_fmri = np.mean(self.fmri, axis=0, keepdims=True)
        self.test_seg_id = rearrange(test_seg_id, 's t -> (s t)') if test_seg_id is not None else None

        self.fmri_transform = fmri_transform
        self.image_transform = image_transform
        self.num_voxels = num_voxels
        self.fpf = self.video.shape[1]  # frames per fmri (v)
    ...
    def __getitem__(self, index):
        if self.window_size:
            ...
            for i in range(self.window_size):
                fmri_to_return = self.fmri[index + i] if t_idx + i < self.t else self.fmri[index + (self.t - t_idx -1)]
                fmri.append(fmri_to_return)
            fmri = np.stack(fmri) # (window_size, num_voxels)
        else: 
            fmri = self.fmri[index]
            fmri = np.expand_dims(fmri, axis=0) # (1, num_voxels)

        img = self.video[index % len(self.video)] / 255.0
        text = [t for t in self.text[index % len(self.text)].reshape(-1)]
        if self.test_seg_id is not None:
            return {'fmri': self.fmri_transform(fmri), 
                    'image': self.image_transform(img), 
                    'text': text, 
                    'uncon_fmri': self.fmri_transform(self.uncon_fmri.repeat(self.window_size, axis=0)),
                    'seg_id': self.test_seg_id[index]}
        else:
            return {'fmri': self.fmri_transform(fmri), 
                    'image': self.image_transform(img), 
                    'text': text, 
